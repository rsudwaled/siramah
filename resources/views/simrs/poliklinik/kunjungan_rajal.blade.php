        ...
        <div class="col-md-12">
            <x-adminlte-card title="Filter Data Kunjungan" theme="secondary" collapsible>
                <form action="" method="get">
                    <div class="row">
                        <x-adminlte-select2 fgroup-class="col-md-4" name="kode_unit" label="Poliklinik">
                            @foreach ($unit as $item)
                                <option value="{{ $item->kode_unit }}"
                                    {{ $item->kode_unit == $request->kode_unit ? 'selected' : null }}>
                                    {{ $item->nama_unit }} {{ $item->kode_unit }}
                                </option>
                            @endforeach
                        ...
                        @endphp
                        <x-adminlte-input-date fgroup-class="col-md-4" name="tgl_masuk" label="Tanggal Antrian"
                            :config="$config" value="{{ \Carbon\Carbon::parse($request->tanggal)->format('Y-m-d') }}">
                            <x-slot name="appendSlot">
                                <x-adminlte-button class="btn-sm" icon="fas fa-search" theme="primary"
                                    label="Submit Pencarian" onclick="getKunjunganRajal()" />
                            </x-slot>
                        </x-adminlte-input-date>
                    </div>
                </form>
            </x-adminlte-card>
            <x-adminlte-card title="Kunjungan Pasien Poliklinik" theme="primary" icon="fas fa-info-circle" collapsible>
                @php
                    $heads = ['Tgl Masuk', 'Pasien', 'No BPJS', 'Penjamin', 'SEP', 'Unit', 'Action'];
                    $config['paging'] = false;
                    $config['info'] = false;
                    $config['scrollY'] = '400px';
                @endphp
                <x-adminlte-datatable id="tableKunjungan" class="nowrap text-xs" :heads="$heads" :config="$config"
...
@section('js')
    <script>
        function getKunjunganRajal() {
            var kodeunit = $('#kode_unit').val();
            var tglmasuk = $('#tgl_masuk').val();
            var url = "{{ route('get_kunjungan_rajal') }}?kode_unit=" + kodeunit + "&tgl_masuk=" + tglmasuk;
            ...
            $.ajax({
                ...
                if (data.metadata.code == 200) {
                    $.each(data.response, function(key, value) {
                        console.log(value.kodebooking);
                        if (value.surat_kontrol) {
                            var btnsuratkontrol =
                                ' <button class="btn btn-xs btn-success" onclick="tambahSuratKontrol(this)" data-nomorkartu="' +
                                value.no_Bpjs + '" data-nama="' + value.nama_px +
                                '" data->Surat Kontrol</button> ';
                        } else {
                            var btnsuratkontrol =
                                ' <button class="btn btn-xs btn-warning">Surat Kontrol</button> ';
                        }
                        if (value.kodebooking) {
                            var btnpanggil =
                                ' <button class="btn btn-xs btn-success" onclick="panggilPasien(this)" data-kodebooking="' +
                                value.kodebooking + '">Panggil</button> '

                        } else {
                            var btnpanggil =
                                ' <button class="btn btn-xs btn-success" onclick="createAntrian(this)" data-kodekunjungan="' +
                                value.kode_kunjungan + '">Panggil</button> '
                        }
        ...
        function panggilPasien(button) {
            alert($(button).data('kodebooking'))
        }

        function createAntrian(button) {
            alert('kode kunjungan ' + $(button).data('kodekunjungan'))
        }

        function tambahSuratKontrol(button) {
            $.LoadingOverlay("show");
            $('#btnStore').show();
            $('#btnUpdate').hide();
