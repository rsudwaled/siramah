...
@section('content')
    <div class="row">
                ...
                <form action="" method="get">
                    <div class="row">
                        ...
                        <x-adminlte-date-range fgroup-class="col-md-4" igroup-size="sm" name="tanggal"
                            label="Tanggal Rawat Inap (Tanggal Pulang)" :config="$config" value="{{ $request->tanggal }}" />
        ...
        @if ($kunjungans)
            <div class="col-md-12">
                <x-adminlte-card theme="primary" icon="fas fa-info-circle"
                    title="Data Pasien Rawat Inap Aktif ({{ $kunjungan_aktif->count() }} Orang)" collapsible="collapsed">
                    @php
                        $heads = ['Tgl Masuk',  'Kunjungan', 'No RM', 'Pasien', 'SEP', 'Alasan Pulang', 'Status','Catatan','Action'];
                        $config['order'] = ['2', 'asc'];
                        $config['paging'] = false;
                    ...
                    <x-adminlte-datatable id="table2" class="nowrap text-xs" :heads="$heads" :config="$config" bordered
                        ...
                        @foreach ($kunjungan_aktif as $item)
                            <tr>
                                ...
                                <td>{{ $item->no_sep }}</td>
                                <td>{{ $item->alasan_pulang->alasan_pulang ?? $item->id_alasan_pulang }}</td>
                                <td>{{ $item->status->status_kunjungan }}</td>
                                <td>{{ $item->catatan }}</td>
                                <td>
                                    <x-adminlte-button class="btn-xs btnKunjungan" theme="primary" icon="fas fa-eye"
                                        label="Action" data-norm="{{ $item->no_rm }}"
                                        data-counter="{{ $item->counter }}" />

                                    @if ($item->status_kunjungan == 1)
                                        <a href="{{ route('tutupkunjungan') }}?kode={{ $item->kode_kunjungan }}"
                                            class="btn btn-xs btn-danger">Tutup Kunjungan</a>
                                    @else
                                        <a href="{{ route('bukakunjungan') }}?kode={{ $item->kode_kunjungan }}"
                                            class="btn btn-xs btn-success">Buka Kunjungan</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
                ...
                <x-adminlte-card theme="success" icon="fas fa-info-circle"
                    title="Data Pasien Rawat Inap Pulang ({{ $kunjungans->count() }} Orang)" collapsible>
                    @php
                        $heads = ['Tgl Masuk', 'Tgl Keluar', 'Kunjungan', 'No RM', 'Pasien', 'SEP', 'Alasan Pulang', 'Status', 'Action'];
                        $config['order'] = ['0', 'asc'];
                        $config['paging'] = false;
                        $config['scrollX'] = true;
                        $config['scrollY'] = '400px';
                            ...
                            <tr>
                                ...
                                <td>
                                    <x-adminlte-button class="btn-xs btnKunjungan" theme="primary" icon="fas fa-eye"
                                        ...
                                        data-counter="{{ $item->counter }}" />
                                    @if ($item->status_kunjungan == 1)
                                        <a href="{{ route('tutupkunjungan') }}?kode={{ $item->kode_kunjungan }}&tanggal={{ $request->tanggal }}"
                                            class="btn btn-xs btn-danger">Tutup Kunjungan</a>
                                    @else
                                        <a href="{{ route('bukakunjungan') }}?kode={{ $item->kode_kunjungan }}&tanggal={{ $request->tanggal }}"
                                            class="btn btn-xs btn-success">Buka Kunjungan</a>
                                    @endif
                                </td>
                            </tr>
