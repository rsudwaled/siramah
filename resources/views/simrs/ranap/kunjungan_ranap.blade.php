            ...
            <x-adminlte-card title="Filter Pasien Rawat Inap" theme="secondary" collapsible>
                <form action="" method="get">
                    <div class="row">
                        ...
                        @endphp
                        <x-adminlte-date-range fgroup-class="col-md-3" igroup-size="sm" name="tanggal"
                            label="Tanggal Rawat Inap" :config="$config" value="{{ $request->tanggal }}" />
                        <x-adminlte-select fgroup-class="col-md-3" igroup-size="sm" name="jenistanggal"
                            label="Jenis Tanggal">
                            <option value="tgl_masuk" {{ 'tgl_masuk' == $request->jenistanggal ? 'selected' : null }}>
                                Tanggal Masuk</option>
                            <option value="tgl_keluar" {{ 'tgl_keluar' == $request->jenistanggal ? 'selected' : null }}>
                                Tanggal Keluar</option>
                        </x-adminlte-select>
                        <x-adminlte-select fgroup-class="col-md-3" igroup-size="sm" name="status" label="Status Pasien">
                            <option value="1" {{ 1 == $request->status ? 'selected' : null }}>Masih Rawat</option>
                            <option value="-" {{ '-' == $request->status ? 'selected' : null }}>Semua Status</option>
                            <option value="2" {{ 2 == $request->status ? 'selected' : null }}>Sudah Pulang</option>
                        </x-adminlte-select>
