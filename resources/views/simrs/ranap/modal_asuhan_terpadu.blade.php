<x-adminlte-modal id="modalAsuhanTerpadu" name="modalAsuhanTerpadu" title="Asuhan Terpadu Pasien" theme="success"
    ...
    <form action="{{ route('simpan_rencana_asuhan_terpadu') }}" name="formAsuhanTerpadu" id="formAsuhanTerpadu"
        ...
        <div class="row">
            <div class="col-md-5">
                ...
                @endphp
                <x-adminlte-input-date name="tgl_waktu" id="tglwaktu" fgroup-class="row" label-class="text-left col-4"
                    igroup-class="col-8" igroup-size="sm" value="{{ now() }}" label="Tgl Waktu"
                    :config="$config" required />
                <x-adminlte-select name="profesi" label="Profesi" fgroup-class="row" label-class="text-left col-4"
                ...
                <x-adminlte-input name="pic" fgroup-class="row" label-class="text-left col-4" igroup-class="col-8"
                    igroup-size="sm" label="Nama Jelas" placeholder="Nama Jelas" value="{{ Auth::user()->name }}"
                    required />
                <x-adminlte-textarea name="rencana_asuhan" label="Rencana Asuhan" rows="3" igroup-size="sm"
                    placeholder="Rencana Asuhan" required>
                </x-adminlte-textarea>
    ...
    <x-slot name="footerSlot">
        <x-adminlte-button theme="success" class="mr-auto" label="Simpan" type="submit" icon="fas fa-save"
            form="formAsuhanTerpadu" />
        <x-adminlte-button theme="danger" label="Dismiss" data-dismiss="modal" />
    </x-slot>
</x-adminlte-modal>
@push('js')
    <script>
        function modalAsuhanTerpadu() {
            $.LoadingOverlay("show");
            $('#modalAsuhanTerpadu').modal('show');
            $.LoadingOverlay("hide");
        }
    </script>
@endpush
