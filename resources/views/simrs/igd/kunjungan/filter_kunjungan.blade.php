...
@section('title', 'Kunjungan Pasien IGD')
@section('content_header')
    <h1>Kunjungan Pasien IGD</h1>
@stop
@section('content')
    <div class="row">
        <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                            <x-adminlte-select2 name="unit" label="Pilih Unit">
                              ...
                              @foreach ($unit as $item)
                                <option value="umum" {{ $request->unit == $item->kode_unit ? 'selected' : '' }}>{{$item->nama_unit}}</option>
                              @endforeach
                            </x-adminlte-select2>
                        </div>
                    </div>
        ...
        @if (isset($kunjungans))
            @if (empty($request->jenis_kunjungan))
                <div class="col-lg-12">
                    <x-adminlte-card theme="primary" collapsible title="Semua Kunjungan">
                        @php
                            $heads = ['Pasien', 'Alamat', 'kunjungan','Tgl Masuk', 'Diagnosa', 'No SEP', 'status', 'action'];
                            $config['ordering'] = false;
                            $config['paging'] = true;
                            $config['info'] = false;
                            $config['searching'] = false;
                            $config['scrollY'] = '650px';
                            ...
                            @foreach ($all as $item)
                                <tr>
                                    <td><b>{{ $item->pasien->nama_px }}</b> <br>RM : {{ $item->no_rm }} <br>NIK :
                                        {{ $item->pasien->nik_bpjs }} <br>BPJS : {{$item->noBpjs}}</td>
                                    ...
                                    <td>{{ $item->kode_kunjungan }} ({{ $item->unit->nama_unit }})</td>
                                    ...
                                    <td>
                                        <x-adminlte-button type="button" data-rm="{{ $item->no_rm }}"
                                            ...
                                            theme="success" class="btn-flat btn-sm" label="detail" />
                                    </td>
                                </tr>
                            @endforeach
            ...
            @if ($request->jenis_kunjungan === 'umum')
                <div class="col-lg-12">
                    <x-adminlte-card theme="primary" collapsible title="Kunjungan Pasien Umum">
                        @php
                            $heads = ['Pasien', 'Alamat', 'kunjungan','Tgl Masuk', 'Diagnosa', 'No SEP', 'status', 'action'];
                            $config['ordering'] = false;
                            $config['paging'] = false;
            ...
            @endif
            @if ($request->jenis_kunjungan === 'bpjs')
                <div class="col-lg-12">
                    <div class="row">
                        <div class="col-lg-7">
                ...
                <div class="col-lg-12">
                    <x-adminlte-card theme="primary" collapsible title="Kunjungan Bridging">
                        ...
                        <x-adminlte-datatable id="table" class="text-xs" :heads="$heads" :config="$config" striped
                            bordered hoverable compressed>
                            @foreach ($kunjunganBridging as $item)
                                <tr>
                                    <td><b>{{ $item->pasien->nama_px }}</b> <br>RM : {{ $item->no_rm }} <br>NIK :
                                        {{ $item->pasien->nik_bpjs }} <br>No Kartu : {{ $item->pasien->no_Bpjs }}</td>
                                    <td>alamat : {{ $item->pasien->alamat }} / <br>
                                        {{ $item->pasien->kode_desa < 1101010001 ? 'ALAMAT LENGKAP BELUM DI ISI!' : $item->pasien->desas->nama_desa_kelurahan . ' , Kec. ' . $item->pasien->kecamatans->nama_kecamatan . ' - Kab. ' . $item->pasien->kabupatens->nama_kabupaten_kota }}
                                    </td>
                                    <td>{{ $item->kode_kunjungan }} ({{ $item->unit->nama_unit }})</td>
