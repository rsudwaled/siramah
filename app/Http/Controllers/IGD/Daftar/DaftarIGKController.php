...
class DaftarIGKController extends Controller
{
   ...
   public function response_no_decrypt($response)
   {
    ...
    public function formIGK( $no,$rm,$jp )
    {
        $antrian        = AntrianPasienIGD::firstWhere('no_antri', $no);
        $pasien         = Pasien::firstWhere('no_rm', $rm);
        $kunjungan      = Kunjungan::where('no_rm', $rm)->get();
        $knj_aktif      = Kunjungan::where('no_rm', $rm)->where('status_kunjungan', 1)->count();
        $alasanmasuk    = AlasanMasuk::whereIn('id', ['4','5','7','8','12','15','16'])->get();
